...
void main() {
  ...
  group('DraftPlugin', () {
    group('Draft Directory Reading', () {
      test(
        'should read posts from _drafts directory and mark them as drafts',
        () async {
          ...
          await memoryFileSystem
              .directory(p.join(projectRoot, '_drafts'))
              .create(recursive: true);
          await memoryFileSystem
...
''');
          ...
          memoryFileSystem
              .file(
                p.join(projectRoot, '_posts', '2024-12-20-published-post.md'),
              )
              .writeAsStringSync('''
---
...
''');
          ...
          // Create draft posts
          memoryFileSystem
              .file(p.join(projectRoot, '_drafts', '2024-12-21-draft-post.md'))
              .writeAsStringSync('''
---
title: Draft Post
date: 2024-12-21
...
''');
          ...
          memoryFileSystem
              .file(
                p.join(projectRoot, '_drafts', '2024-12-22-another-draft.md'),
              )
              .writeAsStringSync('''
---
title: Another Draft
...
''');
          ...
          expect(
            posts.length,
            equals(3),
            reason: 'Should have 1 published + 2 draft posts',
          );

          // Check published post
          final publishedPost = posts.firstWhere(
            (post) => post.config['title'] == 'Published Post',
          );
          expect(
            publishedPost.isDraft(),
            isFalse,
            reason: 'Published post should not be marked as draft',
          );

          // Check draft posts
          final draftPost = posts.firstWhere(
            (post) => post.config['title'] == 'Draft Post',
          );
          expect(
            draftPost.isDraft(),
            isTrue,
            reason: 'Draft post should be marked as draft',
          );
          expect(
            draftPost.config['draft'],
            isTrue,
            reason: 'Draft flag should be set in config',
          );

          final anotherDraft = posts.firstWhere(
            (post) => post.config['title'] == 'Another Draft',
          );
          expect(
            anotherDraft.isDraft(),
            isTrue,
            reason: 'Another draft should be marked as draft',
          );
          expect(
            anotherDraft.config['draft'],
            isTrue,
            reason: 'Draft flag should be set in config',
          );
        },
...
''');
        ...
        // Create regular post
        memoryFileSystem
            .file(p.join(projectRoot, '_posts', '2024-12-20-published-post.md'))
            .writeAsStringSync('''
---
title: Published Post
date: 2024-12-20
...
''');
        ...
        expect(
          posts.length,
          equals(1),
          reason: 'Should have only the published post',
        );
        ...
        expect(
          post.isDraft(),
          isFalse,
          reason: 'Post should not be marked as draft',
        );
      });
...
''');
        ...
        memoryFileSystem
            .file(
              p.join(projectRoot, '_posts', '2024-12-21-frontmatter-draft.md'),
            )
            .writeAsStringSync('''
---
...
''');
        ...
        memoryFileSystem
            .file(
              p.join(projectRoot, '_drafts', '2024-12-22-directory-draft.md'),
            )
            .writeAsStringSync('''
---
...
''');
        ...
        expect(
          posts.length,
          equals(1),
          reason: 'Should only have published post after filtering',
        );
        ...
        expect(
          post.isDraft(),
          isFalse,
          reason: 'Remaining post should not be draft',
        );
      });
...
''');
        ...
        expect(
          posts.length,
          equals(3),
          reason: 'Should have all posts when publish_drafts is true',
        );
        ...
        expect(
          publishedPost.isDraft(),
          isFalse,
          reason: 'Published post should not be draft',
        );

        final frontmatterDraft = posts.firstWhere(
          (post) => post.config['title'] == 'Front Matter Draft',
        );
        expect(
          frontmatterDraft.isDraft(),
          isTrue,
          reason: 'Front matter draft should be marked as draft',
        );

        final directoryDraft = posts.firstWhere(
          (post) => post.config['title'] == 'Directory Draft',
        );
        expect(
          directoryDraft.isDraft(),
          isTrue,
          reason: 'Directory draft should be marked as draft',
        );
      });
...
''');
        ...
        memoryFileSystem
            .file(
              p.join(projectRoot, '_posts', '2024-12-20-explicit-published.md'),
            )
            .writeAsStringSync('''
---
...
''');
        ...
        expect(
          posts.length,
          equals(1),
          reason: 'Should have the explicitly published post',
        );
        ...
        expect(
          post.isDraft(),
          isFalse,
          reason: 'Post with draft: false should not be marked as draft',
        );
        expect(
          post.config['draft'],
          isFalse,
          reason: 'Draft config should be false',
        );
      });
    });
    ...
    group('Custom Draft Directory', () {
      test('should read from custom draft directory when configured', () async {
        ...
        await memoryFileSystem
            ...
            .create(recursive: true);
        await memoryFileSystem
            .directory(p.join(projectRoot, 'drafts'))
            .create(recursive: true);
        await memoryFileSystem
            .directory(p.join(projectRoot, '_layouts'))
            .create(recursive: true);
...
''');
        ...
        memoryFileSystem
            .file(p.join(projectRoot, 'drafts', '2024-12-21-custom-draft.md'))
            .writeAsStringSync('''
---
title: Custom Draft
date: 2024-12-21
...
''');
        ...
        expect(
          posts.length,
          equals(2),
          reason: 'Should have published post + custom draft',
        );
        ...
        final customDraft = posts.firstWhere(
          (post) => post.config['title'] == 'Custom Draft',
        );
        expect(
          customDraft.isDraft(),
          isTrue,
          reason: 'Custom draft should be marked as draft',
        );
        expect(
          customDraft.config['draft'],
          isTrue,
          reason: 'Draft flag should be set',
        );
      });
    });
...
''');
        ...
        final draftPlugin = Site.instance.plugins
            .whereType<DraftPlugin>()
            .first;
        ...
        expect(
          draftPlugin,
          isNotNull,
          reason: 'DraftPlugin should be available',
        );

        // Test basic functionality - should have at least the front matter draft
        final manualDraftCount = Site.instance.posts
            .where((post) => post.isDraft())
            .length;
        expect(
          manualDraftCount,
          greaterThanOrEqualTo(1),
          reason: 'Should have at least 1 draft post',
        );

        // Plugin methods should work correctly
        expect(
          draftPlugin.draftCount,
          equals(manualDraftCount),
          reason: 'Plugin draft count should match manual count',
        );
        expect(
          draftPlugin.drafts.length,
          equals(draftPlugin.draftCount),
          reason: 'Drafts list should match count',
        );

        // Verify that at least the front matter draft exists (this always works)
        final draftTitles = draftPlugin.drafts
            .map((d) => d.config['title'])
            .toSet();
        expect(
          draftTitles.contains('Front Matter Draft'),
          isTrue,
          reason: 'Should contain front matter draft',
        );
      });
    });
  });
