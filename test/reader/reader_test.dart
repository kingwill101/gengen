...
void main() {
  group('Reader with basic setup', () {
    ...
    test('should load static files', () async {
      ...
      expect(site.staticFiles.any((file) => file.name == 'robots.txt'), isTrue);
      expect(
  ...
  group('Reader with exclude/include', () {
    ...
    test(
      'should not load excluded files and directories with various patterns',
      () async {
        await reader.read();
        ...
        // Simple exclude
        expect(
          site.pages.any((page) => page.name == 'excluded_dir/excluded.md'),
          isFalse,
          reason: "excluded_dir/excluded.md should not be loaded",
        );
        expect(
          site.pages.any((page) => page.name == 'excluded_file.md'),
          isFalse,
        ...
        // Wildcard exclude
        expect(
          site.staticFiles.any((file) => file.name == 'secret.txt'),
          isFalse,
        ...
        expect(
          site.pages.any((page) => page.name == 'notes/meeting_notes.md'),
          isFalse,
        ...
        expect(
          site.posts.any((post) => post.name == '_drafts/new_post.md'),
          isFalse,
    ...
    test(
      ...
      () async {
        ...
        expect(
          site.pages.any((page) => page.name == 'notes/important_notes.md'),
          isTrue,
    ...
    test(
      'should exclude directory contents when directory name is excluded (without /**)',
      () async {
        ...
        expect(
          newSite.staticFiles.any((file) => file.name == 'secret/file.txt'),
          isFalse,
          reason:
              "secret/file.txt should be excluded when 'secret' directory is excluded",
        );
        expect(
          newSite.staticFiles.any((file) => file.name == 'secret/config.json'),
          isFalse,
          reason:
              "secret/config.json should be excluded when 'secret' directory is excluded",
        );
        expect(
          newSite.pages.any((page) => page.name == 'secret/nested/deep.md'),
          isFalse,
          reason:
              "secret/nested/deep.md should be excluded when 'secret' directory is excluded",
        );

        // Verify that other files are still included
        expect(
          newSite.pages.any((page) => page.name == 'index.md'),
          isTrue,
    ...
    test('should handle complex exclude-then-include patterns', () async {
      ...
      expect(
        newSite.staticFiles.any((file) => file.name == 'node_modules/index.js'),
        isFalse,
        reason: "node_modules/index.js should be excluded",
      );
      expect(
        newSite.staticFiles.any(
          (file) => file.name == 'node_modules/some-package/dist/other.js',
        ),
        isFalse,
      ...
      expect(
        newSite.staticFiles.any(
          (file) => file.name == 'node_modules/package.json',
        ),
        isTrue,
        reason:
            "node_modules/package.json should be included despite node_modules/** being excluded",
      );
      expect(
        newSite.staticFiles.any(
          (file) => file.name == 'node_modules/some-package/dist/important.css',
        ),
        isTrue,
        reason:
            "node_modules/some-package/dist/important.css should be included despite node_modules/** being excluded",
      );

      // Verify the original include still works
      expect(
        newSite.pages.any((page) => page.name == 'notes/important_notes.md'),
        isTrue,
  ...
  group('Reader with strict front matter', () {
    ...
    test(
      'should skip posts without front matter when strict_front_matter is true',
      () async {
        await reader.read();

        expect(
          site.posts.any((post) => post.config['title'] == 'With Matter'),
          isTrue,
        );
        expect(
